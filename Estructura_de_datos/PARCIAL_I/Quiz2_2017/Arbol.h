...
class Arbol
{
	...
	public:
		...
		enum TipoImpresion{ PREORDEN, INORDEN, POSORDEN };	
		void convertir(){
		
				this->desequilibrado = this->raiz;
				this->equilibrar();
		...
		void imprimir( NodoA<T>* , TipoImpresion = PREORDEN ) const; // imprime la ListaD en el flujo std de salida
		...
		void anchura(NodoA<T>*);
	//	void getHojas( NodoA<T>*, lista_enlazada<T>* ) const; // devuelve una lista de hojas
		int FEnodo(NodoA<T>* recor, int &cont);
		void rotaciones(NodoA<T>* recor);
		void comparandod(NodoA<T>* recor);
		void comparandoi(NodoA<T>* recor);
		void girard(NodoA<T>* recor);
 ...
 */
  template <class T>
 void Arbol<T>::girard(NodoA<T>* recor){
 		this->desequilibrado= recor->getLink_hd();
 		rotacionSD(recor->getLink_hd());
 }
 template <class T>
 void Arbol<T>::comparandod(NodoA<T>* recor){
 	
 		if(recor->getLink_hd()){
 		
 			dere= recor->getLink_hd();
			// 	cout<<"dere "<<*recor->getInfo()<<endl;
			 	der=true;
		 }else{
		 	der=false;
		 }
  ...
  template <class T>
 void Arbol<T>::comparandoi(NodoA<T>* recor){
 	
 		if(recor->getLink_hi()){
 			sigi= recor->getLink_hi();
 			izq=true;
 		//	cout<<"izq"<<*recor->getInfo()<<endl;
		 }else{
		 	izq=false;
		 }
 ...
 template <class T>
 int Arbol<T>::FEnodo(NodoA<T>* recor, int &cont){
 
 		
	//	cout<<" "<<*recor->getInfo();
 	//	cout<<"factor"<<recor->getFactorE()<<endl;
 		if(recor->getFactorE()<=-2 ||recor->getFactorE()>= 2 ){
 		
 			 cont++;
 			 
		 }
 			
 		if(recor->getLink_hi())	FEnodo(recor->getLink_hi(), cont);
 		if(recor->getLink_hd())	FEnodo(recor->getLink_hd(), cont);
 ...
 template <class T>
 void Arbol<T>:: rotaciones(NodoA<T>* recor){
 	
 	if(recor->getLink_hi()){
 		this->izq= true;
 		this->sigi= recor->getLink_hi();
 	
 	//	rotaciones(recor->getLink_hi());
	 }else{
	 	this->izq= false;
	 }
 	...
 	if(recor->getLink_hd()){
 		this->der= true;
 		this->dere= recor->getLink_hd();
 	//	rotaciones(recor->getLink_hd());
	 }else{
	 	this->der= false;
	 }
 ...
 template<class T> 
 void Arbol<T>:: anchura(NodoA<T>* ancho){
 	
 	Colas_dinamicas< NodoA<T>* > colita;
 	
 	colita.Agregar(ancho);
 	...
 	while(!colita.vacia()){
 		...
 		if(aux->getLink_hi()){ 			
 			colita.Agregar(aux->getLink_hi()); 			
	    }
		if(aux->getLink_hd()){
 			colita.Agregar(aux->getLink_hd());
 	    }
 ...
 */
template<class T> void Arbol<T>::imprimir( NodoA<T>* recor, TipoImpresion tipo ) const
{
	// si recor es NULL
	if( !recor )
	{
	...
	if ( recor->getLink_hi() ) // paso recursivo para actualizar al hijo por la izquierda
		imprimir( recor->getLink_hi(), tipo );
		
	
	
	if( tipo == this->PREORDEN )
	{
		for( int i = 0; i < recor->getNivel(); i++ )
		std::cout << "       ";
	...
	// si el NodoA recor tiene hijo por la derecha
	if ( recor->getLink_hd() ) // paso recursivo para actualizar al hijo por la derecha
		imprimir( recor->getLink_hd(), tipo );
		
		
		
	if( tipo == this->POSORDEN )
	{
		for( int i = 0; i < recor->getNivel(); i++ )
		std::cout << "      ";
...
/**
 *	@brief retorna una lista de las hojas del arbol
 *	@since: 1.1
 *	@param NodoA<T>* NodoA donde inicia la busqueda
 *	@param lista de hojas
 */
/*template<class T> void Arbol<T>::getHojas( NodoA<T>* recor, lista_enlazada<T>* hojas )const
{
	// si recor es NULL o el nivel es mayor a la altura del arbol
	if( !recor )
	{
	...
	if( recor->isHoja() )
		hojas->agregar( recor->getInfo() );
		
		
	// si el NodoA recor tiene hijo por la izquierda 
	if ( recor->getLink_hi() ) // paso recursivo para actualizar al hijo por la izquierda
		getHojas( recor->getLink_hi(), hojas );
	
	
	// si el NodoA recor tiene hijo por la derecha
	if ( recor->getLink_hd() ) // paso recursivo para actualizar al hijo por la derecha
		getHojas( recor->getLink_hd(), hojas );
} // fin de getHojas
